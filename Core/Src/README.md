...
`advance_motion.c` 在同一个 20 ms `AdvanceMotion_Update()` 中执行单点与连续路径控制。路径模式在有限前向窗口中更新单调投影进度，并沿折线弧长插值得到动态前视点；前方曲率、航向梯度和末端制动距离共同限制参考速度，参考速度与前视距离均按周期做变化率限制。
路径中段不复用到点平移 PID：前视段切向前馈负责前进，当前投影段法向 PD 负责回到路径，投影进度插值 yaw 与航向变化率前馈共同产生角速度。剩余距离不大于 60 mm 且参考、实测平移速度均不大于 150 mm/s 后，控制器才进入最终 Goto PID 捕获。路径超时按总弧长估算，无进展保护按累计弧长判断。
