...
TIM6 每 1 ms 推进通信状态，按 10 ms 更新传感器与世界位姿，按 20 ms 更新电机通信并检查位姿外环 PID 的待生效配置；位姿外环仅在 WORLD 控制权下输出速度，视觉控制仅在 VISUAL 控制权下输出速度。视觉阻塞接口在自身内部每 20 ms 调度 `AdvanceVisual_Update()`，不再由 TIM6 或主循环调用；其余后台任务仍由 TIM6 统一推进。主循环不消费任务标志，也不主动调用任何后台 Update。
