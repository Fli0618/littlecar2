...
- `sensor_limit`：升降轴顶部归零光电读取模块；只提供 `LIFT_UP_LIMIT_*` 的原始电平和有效状态接口，不负责 GPIO 初始化、中断或电机控制。其他限位宏仍由 CubeMX 保留，但业务不读取。
...
- `advance_arm`：固定 ID 和编译期动作参数的阻塞式机械臂执行器；升降轴使用顶部单光电归零、反馈闭环和 `ARM_LIFT_POS_MAX` 软件行程限制，滑台使用人工软件零点和 `ARM_SLIDE_POS_MAX`。
...
- `sensor_limit` 只读取升降顶部 `LIFT_UP_LIMIT_*` 光电；`SENSOR_LIMIT_ACTIVE_LEVEL` 默认高电平有效，可在 `sensor_limit.h` 中改为低电平有效。`LIFT_DOWN_LIMIT_*`、`SLIDE_FRONT_LIMIT_*` 和 `SLIDE_REAR_LIMIT_*` 仅作为硬件余量保留。
