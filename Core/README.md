...
- `advance_motion`：世界速度、`GotoPose` 与连续路径异步状态机；`FollowPathEx()` 直接引用调用方长期持有的离散路径数组，在中间采样点保持连续运动，仅在最后一点复用 `GotoPose` 的精确收敛与停车逻辑；由 `main.c` 每 20 ms 调度。组合 GOTO 的大航向误差先对准策略由 `advance_motion.h` 的默认宏决定，并可在空闲状态由调参工具临时切换。
