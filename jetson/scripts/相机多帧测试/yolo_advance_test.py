...
from vision import CAMERA_INDEX_COLOR_CIRCLE, advance_detect_circle


WINDOW_NAME = "Advance YOLO Circle Detection | Press Q or ESC to exit"


def main() -> None:
    camera = cv2.VideoCapture(CAMERA_INDEX_COLOR_CIRCLE, cv2.CAP_DSHOW)
    if not camera.isOpened():
        raise RuntimeError(f"无法打开相机: {CAMERA_INDEX_COLOR_CIRCLE}")
    ...
    try:
        while True:
            ...
            result = advance_detect_circle(frame_bgr)
            ...
            cv2.putText(
                result_frame,
                f"Advance YOLO: {len(result['detections'])} tracked, {inference_ms:.1f} ms",
                (20, 36),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.75,
                (0, 255, 0),
