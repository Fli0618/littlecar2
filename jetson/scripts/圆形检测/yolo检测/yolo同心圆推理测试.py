...
# 绘制图表及柱状图时采用的高对比度专业物理配色
CHART_COLORS = ["dodgerblue", "darkorange", "seagreen"]
# ==================================================
        ...
        if gt_id not in used_gts and pred_id not in used_preds:
            if dist < max_dist_thresh:
                ...
                matches.append({
                    ...
                    "gt": gt,
                    "pred": pred,
                    "class_correct": gt["class_name"] == pred["class_name"]
                })
...
def main() -> None:
    print(f"尝试加载目标检测模型: {MODEL_PATH}")
    if not MODEL_PATH.exists():
        print(f"未能在预设位置找到模型权重文件，请核对绝对路径或相对路径。")
        return
    ...
    if not all_images:
        print(f"在路径 '{DATASET_DIR / 'images'}' 下未发现测试图像，请确保已经通过生成脚本构建了数据集。")
        return

    # 确定测试数量
    test_count = min(NUM_TEST_IMAGES, len(all_images))
    selected_img_paths = random.sample(all_images, test_count)
    print(f"开始压力测试评估，随机抽样数量: {test_count} 张图像...\n")

    num_classes = len(CLASS_NAMES)
    total_counts = [0] * num_classes    # 每种类型真实出现的总次数
    class_errors = [0] * num_classes    # 每种类型定位到了但分类错误的次数
    missed_counts = [0] * num_classes   # 每种类型完全漏检的次数
    
    wrong_examples = []  # 存放包含“分类错误/漏检/误检”的绘图和文件名
    infer_times = []
    ...
    for i, path_str in enumerate(selected_img_paths, 1):
        ...
        if i % 10 == 0 or i == test_count:
            print(f"进度：已推理完成并计算 {i}/{test_count} 张图像...")

    print(f"\n==================================================")
    print(f"压测结束！共分析 {test_count} 张图 | 累计平均推理耗时: {sum(infer_times)/len(infer_times):.2f} ms")
    print(f"检测出现异常（分类错/漏检/虚警）的图像数: {len(wrong_examples)} / {test_count}")
    print(f"==================================================\n")

    # --- 1. 使用 Matplotlib 绘制统计图表 ---
    ...
    ax1.set_xlabel("Encoder Target Number")
    ...
    ax2.set_xlabel("Encoder Target Number")
    ...
    plt.suptitle("Batch Performance & Failures Diagnostics (3-Class Encoder Scheme)", fontsize=13, y=0.98)
    plt.tight_layout()
    plt.show()
    ...
    if wrong_examples:
        ...
        plt.suptitle(f"Selected Error & Missed Examples ({len(wrong_examples)} failed images found in total)", fontsize=12, y=0.98)
        plt.tight_layout()
        plt.show()
    else:
        print("🎉 压测结果良好：在当前抽样测试集上未发现任何分类错误或漏检样本！")
