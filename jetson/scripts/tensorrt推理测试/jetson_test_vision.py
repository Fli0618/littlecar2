"""Jetson TensorRT vision test: run circle and color detection on one frame."""
...
def main() -> None:
    ...
    if not camera.isOpened():
        camera.release()
        raise RuntimeError(f"cannot open TensorRT vision camera: {CAMERA_DEVICE}")
    ...
    try:
        while True:
            ...
            phase = "warmup" if frame_number == 1 else "steady"
            print(
                f"frame={frame_number} phase={phase} device={CAMERA_DEVICE} "
                f"total_inference_ms={total_ms:.1f}",
                flush=True,
            )
        ...
        print("\nTensorRT vision test stopped.", flush=True)
    finally:
        reset_advance_tracking()
        camera.release()
