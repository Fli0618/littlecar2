...
地图的“显示交换 X/Y”和“显示反转 X/Y”统一控制地图坐标系：实测车辆、轨迹和误差使用正向变换，单点 GOTO、地图流程及连续路径在下发前使用逆变换，原始遥测和已保存方案保持不变。X/Y 反转每次启动默认开启，交换默认关闭；车辆航向随方向向量同步变换。单步执行始终运行当前选中动作，可重复点击；连贯执行从当前选中动作开始。
