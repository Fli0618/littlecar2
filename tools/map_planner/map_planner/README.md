...
`models.py` 定义方案、停点动作、原地转向和连续路径段，并维护旧版本迁移；`geometry.py` 提供坐标变换，`sim.py` 提供停点近似仿真和连续路径几何播放，`storage.py` 负责 JSON 方案存储，`gui.py` 负责 PySide6 地图编辑界面，`codegen_c.py` 按步骤顺序生成 STM32 代码。
