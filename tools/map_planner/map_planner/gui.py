...
PLATFORMS = ((550, 500), (1400, 500), (550, 1450), (1400, 1450))
...
class WaypointItem(QGraphicsEllipseItem):
    def __init__(self, index, x, y, active, moved, rotated, activate):  # type: ignore[no-untyped-def]
        super().__init__(-18, -18, 36, 36); self.index = index; self.moved = moved; self.activate = activate; self._before = QPointF(x, y)
        self.setPos(x, y); self.setBrush(QColor("#4caf50") if active else QColor("#f28c28")); self.setPen(QPen(QColor("#333333"), 3)); self.setZValue(10)
...
class PlannerWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.plan = Plan(); self.active_index = -1; self.mode = "select"; self.calibration_pending = False; self.undo_stack = []
        ...
        self._build(); QShortcut(QKeySequence.StandardKey.Undo, self, activated=self.undo); QShortcut(QKeySequence.StandardKey.SelectAll, self, activated=self.select_all)
        self.redraw(); self.refresh_plans()
    ...
    def _build(self) -> None:
        ...
        box.addLayout(row)
        box.addWidget(QLabel("起点标定")); starts = QHBoxLayout()
        for label in ("启停区 1", "启停区 2", "自定义"):
            button = QPushButton(label); button.clicked.connect(lambda checked=False, value=label: self.begin_start(value)); starts.addWidget(button)
        ...
        root.addWidget(scroll); root.addWidget(self.view); root.setSizes([360,1060]); self.setCentralWidget(root)
    ...
    def begin_start(self, kind):
        ...
        if kind == "自定义": self.mode="calibrate"; self.view.mode="calibrate"; self.status.setText("在地图点击自定义起点位置，再拖动蓝色箭头确定朝向。"); return
        self.plan.start_paper_x_mm, self.plan.start_paper_y_mm = START_PRESETS[kind]; self.redraw(); self.status.setText("已放置起点，请拖动蓝色流程箭头的圆柄标定朝向。")

    def on_map_click(self, x, y, shift=False):
        if not (0 <= x <= FIELD_SIZE_MM and 0 <= y <= FIELD_SIZE_MM): return
        self.push_undo()
        if self.mode == "calibrate":
            self.plan.start_paper_x_mm, self.plan.start_paper_y_mm = x, y; self.calibration_pending=True; self.set_mode("select"); self.redraw(); return
        # 该公开方法也供离屏测试和脚本调用；实际鼠标只会在添加模式触发它。
        if shift and self.plan.waypoints:
            prev=self.paper_of(self.plan.waypoints[-1]); dx=x-prev.x_mm; dy=y-prev.y_mm; angle=math.atan2(dy,dx); snapped=round(angle/(math.pi/4))*math.pi/4; length=math.hypot(dx,dy); x=prev.x_mm+length*math.cos(snapped); y=prev.y_mm+length*math.sin(snapped)
    ...
    def move_waypoint(self,index,before,after,shift=False):
        ...
        self.push_undo(); x,y=after.x(),after.y()
        if shift and index:
            prev=self.paper_of(self.plan.waypoints[index-1]); angle=math.atan2(y-prev.y_mm,x-prev.x_mm); angle=round(angle/(math.pi/4))*math.pi/4; d=math.hypot(x-prev.x_mm,y-prev.y_mm); x,y=prev.x_mm+d*math.cos(angle),prev.y_mm+d*math.sin(angle)
        pose=paper_to_world(x,y,self.plan.start_paper_x_mm,self.plan.start_paper_y_mm,self.plan.start_heading_deg); p=self.plan.waypoints[index]; p.x_mm,p.y_mm=pose.x_mm,pose.y_mm; self.refresh_waypoints(); self.redraw()

    def rotate_waypoint(self,index,item):
        if index != self.active_index: return
        pitem=item.parentItem(); delta=item.pos(); yaw=-math.degrees(math.atan2(delta.y(),delta.x()))-self.plan.start_heading_deg
        p=self.plan.waypoints[index]; p.yaw_deg=yaw; p.use_yaw=True; self.show_node(index); self.redraw()

    def rotate_start(self,item):
        delta=item.pos(); self.plan.start_heading_deg=-math.degrees(math.atan2(delta.y(),delta.x())); self.calibration_pending=False; self.redraw(); self.status.setText("起点标定完成。")

    def select_box(self,rect,append):
        if not append: self.scene.clearSelection()
        for item in self.scene.items(rect):
            if isinstance(item,(WaypointItem,StartItem)):
                item.setFlag(QGraphicsItem.GraphicsItemFlag.ItemIsSelectable, True); item.setSelected(True)
    ...
    def push_undo(self): self.undo_stack.append(copy.deepcopy(self.plan)); self.undo_stack=self.undo_stack[-100:]
    def undo(self):
        if not self.undo_stack:return
        self.plan=self.undo_stack.pop(); self.active_index=min(self.active_index,len(self.plan.waypoints)-1); self.simulation=None; self.actual_trace=[]; self.refresh_waypoints(); self.redraw()
    ...
    def draw_field(self):
        ...
        self.static(self.scene.addRect(0,950,155,500,QPen(Qt.PenStyle.NoPen),QColor("#ffffff")),"storage_base"); self.static(self.scene.addRect(960,2250,480,150,QPen(Qt.PenStyle.NoPen),QColor("#ffffff")),"rough_base")
        self.static(self.scene.addEllipse(1060,-140,280,280,QPen(QColor("#444"),4),QColor("#f7f7f7")),"raw_turntable")
        ...
        self.text(720,105,"原料区","raw_label",26); self.text(180,1260,"暂存区","storage_label",24,90); self.text(1320,2180,"粗加工区","rough_label",26); self.text(2290,1300,"二次编码区","coding_label",22,90); self.text(800,2420,"可移动区域边界：2400 × 2400 mm","movable_boundary_label",18)
    ...
    def draw_car(self,pose):
        p=pose or Pose(); x,y=world_to_paper(p,self.plan.start_paper_x_mm,self.plan.start_paper_y_mm,self.plan.start_heading_deg); yaw=p.yaw_deg+self.plan.start_heading_deg
        car=QGraphicsRectItem(-150,-150,300,300); car.setPos(x,y); car.setRotation(-yaw); car.setPen(QPen(QColor("#b00020"),5)); car.setBrush(QColor(230,40,40,128)); car.setZValue(15); self.scene.addItem(car)
    ...
    def play(self):
        ...
        route=[Pose(0,0,0)]+[Pose(p.x_mm,p.y_mm,p.yaw_deg if p.use_yaw else 0) for p in self.plan.waypoints]; self.simulation=Simulation(route,list(range(1,len(route))),self.plan.settings,self.plan.start_paper_x_mm,self.plan.start_paper_y_mm,self.plan.start_heading_deg); self.actual_trace=[]; self.timer.start()
    ...
    def new_plan(self): self.pause(); self.plan=Plan(); self.active_index=-1; self.actual_trace=[]; self.undo_stack=[]; self.begin_start("启停区 1"); self.refresh_waypoints(); self.redraw()
    def save(self):
    ...
    def load_selected(self):
        ...
        try: self.plan=load_plan(item.text()); self.active_index=-1; self.refresh_waypoints(); self.redraw()
