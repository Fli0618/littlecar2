...
from pid_tuner.models import (GotoControlConfigSnapshot, GotoControlConfigState, GotoStrategySnapshot, PathConfigSnapshot, PathConfigState, PidConfig,
                               PidConfigState, Telemetry)
from pid_tuner.storage import (GOTO_CONTROL_C_MACROS, PATH_C_MACROS, PID_C_MACROS, export_c_defaults,
                               export_motion_config_header, list_profiles, load_profile, save_profile,
                               write_telemetry_csv)


class StorageTests(unittest.TestCase):
    GOTO_CONTROL = GotoControlConfigState(9, GotoControlConfigSnapshot(
    ...
    def test_export_motion_config_header_is_complete_and_deterministic(self) -> None:
        ...
        exported = export_motion_config_header(pid, path, GotoStrategySnapshot(True), self.GOTO_CONTROL)

        macros = re.findall(r"^#define (ADVANCE_MOTION_[A-Z0-9_]+) ", exported, re.MULTILINE)
        expected = [name for name, _ in PID_C_MACROS + PATH_C_MACROS + GOTO_CONTROL_C_MACROS]
        expected.extend(("ADVANCE_MOTION_GOTO_CORRECTION_OPEN_LOOP_MS",
        ...
        self.assertIn("#define ADVANCE_MOTION_GOTO_PROFILE_THRESHOLD_MM (500.0f)", exported)
        self.assertEqual(exported, export_motion_config_header(
            pid, path, GotoStrategySnapshot(True), self.GOTO_CONTROL))
    ...
    def test_export_motion_config_header_writes_false_strategy_and_rejects_nonfinite_values(self) -> None:
        ...
        self.assertIn("((uint8_t)0U)", export_motion_config_header(
            pid, path, GotoStrategySnapshot(False), self.GOTO_CONTROL))
        for invalid in (math.nan, math.inf, -math.inf):
            invalid_path = PathConfigState(2, PathConfigSnapshot(invalid, *path_values[1:]))
            with self.assertRaises(ValueError):
                export_motion_config_header(pid, invalid_path, GotoStrategySnapshot(False), self.GOTO_CONTROL)
