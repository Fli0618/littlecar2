...
启动后使用界面左上方的串口下拉框选择 COM 口并点击“连接”，不需要在启动命令中填写端口。GUI 使用现有世界位姿遥测：X/Y 是经坐标修正后的 OPS 实际位置，yaw 在 WIT 有效时优先使用 WIT，否则使用 OPS。固件启动后持续发送遥测，空闲时也会刷新世界位姿，因此手动拖拽可以直接体现在图表上。GOTO 默认目标为 `(0, 0, 0)`，速度为 `50 mm/s`、角速度为 `30 deg/s`、超时为 `5000 ms`。默认显示最近 30 秒，窗口可调 5-120 秒，缓存只保留最近 120 秒。
