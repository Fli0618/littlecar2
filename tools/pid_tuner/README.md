...
启动后使用界面左上方的串口下拉框选择 COM 口并点击“连接”，不需要在启动命令中填写端口。GUI 持续显示 OPS X/Y、WIT 相对航向和 OPS 相对 Z 航向；两套航向均与同一 GOTO 目标角对比。航向 PID 数据源可在 WIT 与 OPS 之间切换，默认 WIT；切换会重置航向 PID 历史。GOTO 默认目标为 `(0, 0, 0)`，调试最高限速为 `1200 mm/s`、`120 deg/s`，默认 vmax 为 `600 mm/s`，默认超时为 `15000 ms`。除组合 GOTO 外，界面还提供仅位置和仅角度 GOTO 用于独立 PID 调试。重置零点仅在空闲时成功，并会将当前 OPS X/Y、WIT 航向与 OPS 航向同时归零。每次“应用 PID”获得 STM32 确认后，启动 GUI 的终端会输出修订号与六个 PID 参数。默认显示最近 30 秒，窗口可调 5-120 秒，缓存只保留最近 120 秒。
